...
def part1(a):
    ...
    for v in tot:
        gamma = gamma + '1' if int(v)>len(a)/2 else gamma+'0'
        epsilon = epsilon + '0' if int(v) > len(a)/2 else epsilon+'1'
...
def part2(a):
    ...
    for i in range(len(a[0])):
        ...
        predOxy = predOxy + '1' if totOxy >= len(possOxyVals)/2 else predOxy + '0'
        predCO2 = predCO2 + '0' if totCO2 >= len(possCO2Vals)/2 else predCO2 + '1'
